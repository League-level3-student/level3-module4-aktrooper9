...
public class MazeMaker {
	...
	public static Maze generateMaze(int r, int c) {
		...
		// finish line.
		int in = randGen.nextInt(rows);
		int out = randGen.nextInt(rows);
		maze.getCell(in, 0).setWestWall(false);
		maze.getCell(out, cols - 1).setEastWall(false);

		// 2. select a random cell in the maze to start
		Cell cell = maze.getCell(randGen.nextInt(rows), randGen.nextInt(cols));
	...
	private static void selectNextPath(Cell currentCell) {
		...
		ArrayList<Cell> neighbor = getUnvisitedNeighbors(currentCell);
		// C. if has unvisited neighbors,
		if (neighbor.size() > 0) {
			// C1. select one at random.
			int nextCell = randGen.nextInt(neighbor.size());
			Cell newCell = neighbor.get(nextCell);
